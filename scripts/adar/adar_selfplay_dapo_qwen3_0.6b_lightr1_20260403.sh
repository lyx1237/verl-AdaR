#!/bin/bash
# TESTED: YES (2026-04-03, 使用Qwen2.5-0.5B-Instruct+256ctx在a6000-4测试通过, 流程无报错)
# 实验目的: Qwen3-0.6B DAPO Self-Play 训练 (测试运行)
# 数据: qihoo360/Light-R1-SFTData (SFT数据转self-play格式)
# 主要配置: DAPO, 3xGPU(A6000-4, GPU1,2,3), Qwen3-0.6B, Light-R1-SFTData
# 基于 adar_selfplay_dapo_qwen3_4b_80k_20260403.sh 修改:
#   1. 模型: Qwen3-4B -> Qwen3-0.6B
#   2. GPU: 4卡(0,2,3,5) -> 3卡(1,2,3)
#   3. 数据: orca_80k -> Light-R1-SFTData
#   4. batch size等参数适配0.6B小模型和3卡
# 日期: 2026-04-03
...
if [ ! -f "$TRAIN_DATA" ]; then
    ...
    $CONDA_PYTHON "$SCRIPT_DIR/prepare_lightr1_sft_data.py" \
        "$RAW_DATA_DIR" \
        "$TRAIN_DATA"
    if [ $? -ne 0 ]; then
        echo "---ERROR--- 数据准备失败!"
...
LOG_FILE="$LOG_DIR/adar_dapo_0.6b_lightr1_$(date +%Y%m%d_%H%M%S).log"

echo "---TRAIN--- 开始Qwen3-0.6B DAPO Self-Play训练..."
...
$CONDA_PYTHON -m recipe.adar_selfplay.run_adar_selfplay \
    ...
    trainer.experiment_name='qwen3_0.6b_dapo_lightr1_test' \
