...
#   2. GPU: 3卡(A6000-4) -> 4卡(A6000-1, GPU0,1,2,3)
#   3. batch size/lr/offload等参数适配4B模型
#   4. lr: 2e-6 -> 1e-6 (4B模型用更低学习率)
#   5. param_offload/optimizer_offload开启 (4B显存需求更大)
#   6. micro_batch_size降低适配4B
# 日期: 2026-04-03
...
CKPT_DIR="$VERL_DIR/ckpt/adar_dapo_4b_lightr1"
...
LOG_FILE="$LOG_DIR/adar_dapo_4b_lightr1_$(date +%Y%m%d_%H%M%S).log"

echo "---TRAIN--- 开始Qwen3-4B DAPO Self-Play训练 (Light-R1-SFTData)..."
...
$CONDA_PYTHON -m recipe.adar_selfplay.run_adar_selfplay \
    ...
    trainer.experiment_name='qwen3_4b_dapo_lightr1' \
