#!/bin/bash
# TESTED: NO
# 实验目的: Qwen3-4B DAPO Self-Play 训练 (Orca 900样本)
# 数据: orca-math-word-problems-200k 随机采样900条
# 基于 adar_selfplay_dapo_qwen3_4b_lightr1_8gpu_20260403.sh 修改:
#   1. 数据: Light-R1-SFTData -> Orca 900 (简单算术题, 更适合Stage1模板+代码生成)
#   2. experiment_name/ckpt_dir 对应更新
# 日期: 2026-04-04
...
TRAIN_DATA="$VERL_DIR/data/selfplay/train_orca_900.parquet"
CKPT_DIR="$ADAR_DIR/ckpt/adar_dapo_4b_orca900"
...
LOG_FILE="$LOG_DIR/adar_dapo_4b_orca900_$(date +%Y%m%d_%H%M%S).log"

echo "---TRAIN--- 开始Qwen3-4B DAPO Self-Play训练 (Orca 900)..."
...
$VENV_PYTHON -m recipe.adar_selfplay.run_adar_selfplay \
    ...
    trainer.experiment_name='qwen3_4b_dapo_orca900' \
